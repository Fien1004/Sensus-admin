...
.insights-view { padding: 0 var(--space-5) var(--space-5); display:flex; flex-direction:column; gap:24px }
.page-header { display:flex; align-items:flex-start; justify-content:space-between; gap:var(--space-5); padding-top:6px }
...
.kpi-card { background:var(--color-surface); border:1px solid var(--color-border); border-radius:12px; padding:24px; box-shadow:var(--shadow-sm); height:106px; display:flex; flex-direction:column; justify-content:center; align-items:flex-start }
...
.selector-and-overview { display:grid; grid-template-columns: 1fr; gap:16px }
.selector-card .select-input { width:100%; border:1px solid var(--color-border); border-radius:14px; padding:18px 20px; background:var(--color-surface); box-sizing:border-box }
.cards-row { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; align-items: stretch }
.selector-wrapper { margin-bottom: 0 }
.insights-card, .behavior-card, .reflections-card, .behavior-breakdown, .drop-per-step { background:var(--color-surface); border:1px solid var(--color-border); border-radius:14px; padding:18px; box-shadow:var(--shadow-sm); display:flex; flex-direction:column }

/* Ensure the two cards in the cards-row stretch to equal height */
.cards-row > .insights-card,
.cards-row > .behavior-card {
  flex: 1 1 auto;
}
...
.small-card { background:var(--color-surface); border:1px solid var(--color-border); border-radius:12px; padding:12px }
.small-label { color:var(--color-neutral-700); font-size:13px }
.small-value { font-size:22px; font-weight:700 }
.warning-box { grid-column: 1 / -1; background: rgba(255, 235, 230, 0.9); border:1px solid rgba(255,180,160,0.5); padding:12px; border-radius:10px; color:var(--color-danger) }
.behavior-grid-vertical { display: grid; grid-template-columns: 1fr 1fr; gap: 12px }
.behavior-card.small { padding:12px }

/* Ensure the outer behavior card matches the height of the insights card and
  places the note at the bottom */
.overview-grid > aside.behavior-card { display: flex; flex-direction: column; justify-content: space-between }
...
.gender-card { border-radius: 12px; border: 1px solid var(--color-border); background: var(--color-surface); padding: 10px; height: 107px; box-shadow: var(--shadow-sm); display:flex; flex-direction:column }
.gender-title { font-weight:700; margin-bottom:8px }
.progress-bar-container { display:flex; align-items:center }
.progress-full { width:100%; background:var(--color-neutral-200); border-radius:8px; overflow:hidden; display:flex; height:46px }
.segment { height:46px; display:flex; align-items:center; padding-left:12px; font-size:14px; font-weight:700; line-height:22px; white-space:nowrap }
...
.steps-outer { background: var(--color-surface); border-radius: 12px; border: 1px solid var(--color-border); padding: 24px 8px; box-shadow: var(--shadow-sm) }
.step-row { display: grid; grid-template-columns: 180px 1fr 90px; align-items: center; column-gap: 24px; padding: 24px 8px; border-bottom: 1px solid var(--color-border) }
...
.bar-a, .bar-b { height:46px; border-radius:8px; display:flex; align-items:center; padding:0 30px; color:#fff; font-weight:700; font-size:14px; white-space:nowrap }
...
.drop-card { width:90px; min-height:72px; border-radius:8px; background:var(--color-neutral-100); display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center }
